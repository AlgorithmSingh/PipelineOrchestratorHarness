...
export function validateExecutionReadiness(config: HarnessConfig): void {
	...
	if (!Array.isArray(checks) || checks.length === 0) {
		throw new ConfigError(missingChecksMessage(), {
			field: "pipelines.execution.checks",
		});
	}
...
export async function loadConfig(
	...
	const projectConfigPath = targetProject
		? join(resolve(targetProject), ".harness/config.yaml")
		: undefined;
	...
	if (
		...
		throw new ConfigError(`Missing config file at ${configPath}`, {
			configPath,
		});
	}
	...
	const summariesDir = resolve(effectiveRoot, ".harness/summaries");
